...
const float LIGHT_MIN			 = -180.0;
const float LIGHT_MAX			 = 180.0;

// ***************** LIGHT PARAMETERS *********************
float LIGHT_ANGLE = 0.0;
const float LIGHT_RADIUS = 4.0;
const float LIGHT_Z = 5.0;
...
GLuint normalbuffer_landscape;

GLuint vertexbuffer_head;
GLuint uvbuffer_head;
GLuint normalbuffer_head;

GLuint vertexbuffer_body;
GLuint uvbuffer_body;
GLuint normalbuffer_body;
...
std::vector<glm::vec3> vertices_landscape;
std::vector<glm::vec2> uvs_landscape;
std::vector<glm::vec3> normals_landscape;

std::vector<glm::vec3> vertices_head;
std::vector<glm::vec2> uvs_head;
std::vector<glm::vec3> normals_head;

std::vector<glm::vec3> vertices_body;
std::vector<glm::vec2> uvs_body;
std::vector<glm::vec3> normals_body;

std::vector<glm::vec3> vertices_background;
std::vector<glm::vec2> uvs_background;
...
void setupShaders()
{
    ...
    programID = LoadShaders( "shader/light.vertexshader", "shader/light.fragmentshader" );
    printf("shader programID is: %d\n", programID);
...
void setupTextures()
{
    texture_landscape = loadBMP_custom("mesh/snow.bmp");
    texture_background = loadBMP_custom("mesh/galaxy.bmp");
    //background_texture = loadBMP_custom("mesh/head.bmp");
}

void setupObjects()
{
    /*************** LOADING LANDSCAPE ****************/
    // the obj file for landscape is obtained from: http://www.badking.com.au/site/shop/environment/ring-doom-terrain/
    bool loaded_landscape = loadOBJ("mesh/landscape.obj", vertices_landscape, uvs_landscape, normals_landscape);

    if(loaded_landscape)
    {
        printf("landscape loaded\n");
    }

    // Load it into a VBO
    glGenBuffers(1, &vertexbuffer_landscape);
    glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer_landscape);
    glBufferData(GL_ARRAY_BUFFER, vertices_landscape.size() * sizeof(glm::vec3), &vertices_landscape[0], GL_STATIC_DRAW);

    glGenBuffers(1, &uvbuffer_landscape);
    glBindBuffer(GL_ARRAY_BUFFER, uvbuffer_landscape);
    glBufferData(GL_ARRAY_BUFFER, uvs_landscape.size() * sizeof(glm::vec2), &uvs_landscape[0], GL_STATIC_DRAW);

    glGenBuffers(1, &normalbuffer_landscape);
    glBindBuffer(GL_ARRAY_BUFFER, normalbuffer_landscape);
	glBufferData(GL_ARRAY_BUFFER, normals_landscape.size() * sizeof(glm::vec3), &normals_landscape[0], GL_STATIC_DRAW);

    /*************** LOADING HEAD ****************/
    bool loaded_head = loadOBJ("mesh/head.obj", vertices_head, uvs_head, normals_head);
    ...
    glGenBuffers(1, &vertexbuffer_head);
    glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer_head);
    glBufferData(GL_ARRAY_BUFFER, vertices_head.size() * sizeof(glm::vec3), &vertices_head[0], GL_STATIC_DRAW);

    glGenBuffers(1, &uvbuffer_head);
    glBindBuffer(GL_ARRAY_BUFFER, uvbuffer_head);
    glBufferData(GL_ARRAY_BUFFER, uvs_head.size() * sizeof(glm::vec2), &uvs_head[0], GL_STATIC_DRAW);

    glGenBuffers(1, &normalbuffer_head);
    glBindBuffer(GL_ARRAY_BUFFER, normalbuffer_head);
	glBufferData(GL_ARRAY_BUFFER, normals_head.size() * sizeof(glm::vec3), &normals_head[0], GL_STATIC_DRAW);

	/*************** LOADING BODY ****************/
	bool loaded_body = loadOBJ("mesh/body.obj", vertices_body, uvs_body, normals_body);
	...
	glGenBuffers(1, &vertexbuffer_body);
    glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer_body);
    glBufferData(GL_ARRAY_BUFFER, vertices_body.size() * sizeof(glm::vec3), &vertices_body[0], GL_STATIC_DRAW);

    glGenBuffers(1, &uvbuffer_body);
    glBindBuffer(GL_ARRAY_BUFFER, uvbuffer_body);
    glBufferData(GL_ARRAY_BUFFER, uvs_body.size() * sizeof(glm::vec2), &uvs_body[0], GL_STATIC_DRAW);

    glGenBuffers(1, &normalbuffer_body);
    glBindBuffer(GL_ARRAY_BUFFER, normalbuffer_body);
	glBufferData(GL_ARRAY_BUFFER, normals_body.size() * sizeof(glm::vec3), &normals_body[0], GL_STATIC_DRAW);

    /*************** LOADING BACKGROUND ****************/
    bool loaded_background = loadOBJ("mesh/background.obj", vertices_background, uvs_background, normals_background);

    if(loaded_background)
    {
        printf("background plane loaded\n");
    }

    glGenBuffers(1, &vertexbuffer_background);
    glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer_background);
    glBufferData(GL_ARRAY_BUFFER, vertices_background.size() * sizeof(glm::vec3), &vertices_background[0], GL_STATIC_DRAW);

    glGenBuffers(1, &uvbuffer_background);
    glBindBuffer(GL_ARRAY_BUFFER, uvbuffer_background);
    glBufferData(GL_ARRAY_BUFFER, uvs_background.size() * sizeof(glm::vec2), &uvs_background[0], GL_STATIC_DRAW);

    glGenBuffers(1, &normalbuffer_background);
    glBindBuffer(GL_ARRAY_BUFFER, normalbuffer_background);
	glBufferData(GL_ARRAY_BUFFER, normals_background.size() * sizeof(glm::vec3), &normals_background[0], GL_STATIC_DRAW);
...
void initGlui()
{
	...
	glui_spinner->set_speed(SPINNER_SPEED*0.6);

	///////////////////////////////////////////////////////////
	// DONE (for controlling light source position & additional
	//      rendering styles):
...
Vector getInterpolatedJointDOFS(float time)
{
	...
	if( i == 1 )
	{
        //special case 1: beginning point
	...
	Vector v0 = p0;
	Vector v1 = t0 * time;
	Vector v2 = (p0 * (-3) + p1 * 3 + t0 * (-2) + t1 * (-1)) * pow(time, 2);
	Vector v3 = (p0 * 2 + p1 * (-2) + t0 + t1) * pow(time, 3);
...
void drawOlaf(void)
{
    ...
    glPushMatrix();
		...
		drawObj(vertexbuffer_head, uvbuffer_head, normalbuffer_head, vertices_head);
    glPopMatrix();
