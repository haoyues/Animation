...
float* normalVector(float p1[], float p2[], float p3[]);
void drawPolygon3D(const float *vertices, int numofVertices);
void drawPenguin();
void drawCube();
...
void display(void)
{
    ...
    else if(renderStyle == SOLID || renderStyle == OUTLINED)
    {
        glDisable(GL_LIGHTING);
    ...
    {
        // got data from http://devernay.free.fr/cours/opengl/materials.html
        // brass
        float ambient[3]= {0.329412, 0.223529, 0.027451};
        float diffuse[3] = {0.780392, 0.568627, 0.113725};
        float specular[3] = {0.992157, 0.941176, 0.807843};
    ...
    if(renderStyle == MATTE)
    {
        // got data from http://devernay.free.fr/cours/opengl/materials.html
        // red rubber
        float ambient[3] = { 0.05,0.0,0.0};
        float diffuse[3] = { 0.5,0.4, 0.4 };
        float specular[3] = { 0.7, 0.04, 0.04 };
        float shininess = 0.078125;
...
// shoulder roll: rotation along x axis
// shoulder pitch: rotation along y aixs
// shoulder yaw: rotation along z axis
void drawPenguin()
{
    ...
    glPushMatrix();
        ...
        // fixed rotation order about the local Z, Y and X
        ...
        glPushMatrix();
            ...
            glPushMatrix();
                ...
                drawPolygon3D(footVertices, 5);
            glPopMatrix();
        glPopMatrix();

        /*************************** RIGHT HIP ****************************/
        glPushMatrix();
            glTranslatef(-0.5, -1.9, -0.2);
            // fixed rotation order about the local Z, Y and X
            glTranslatef(0.0, 0.5, 0.0);
...
/*************************** DRAWING FUNCTIONS **************************/
// function for getting the normal of the surface, given 3 points on the surface
float* normalVector(float p1[], float p2[], float p3[])
{
	...
	float v2_x = p3[0] - p2[0];
	float v2_y = p3[1] - p2[1];
	float v2_z = p3[2] - p2[2];

	float normalVector[3];
	normalVector[0] = v1_y * v2_z - v1_z * v2_y;
	normalVector[1] = v1_z * v2_x - v1_x * v2_z;
	normalVector[2] = v1_x * v2_y - v1_y * v2_x;
	float length = sqrtf(powf(normalVector[0], 2) + powf(normalVector[1], 2) + powf(normalVector[2], 2) );
...
// General function to draw polygon
void drawPolygon3D(const float *vertices, int numofSurfaces)
{
    int i;
    int j;
    int vertexIdx = 0;
    ...
    glBegin(GL_QUADS);
        ...
        {
            // get 3 points on the surface
            p1[0] = vertices[vertexIdx++];
            p1[1] = vertices[vertexIdx++];
            p1[2] = vertices[vertexIdx++];

            p2[0] = vertices[vertexIdx++];
            p2[1] = vertices[vertexIdx++];
            p2[2] = vertices[vertexIdx++];

            p3[0] = vertices[vertexIdx++];
            p3[1] = vertices[vertexIdx++];
            p3[2] = vertices[vertexIdx++];

            //calculate the normal
            normal = normalVector(p1, p2, p3);
            ...
            for(j = 0; j < 4; j++)
            {
                glVertex3f(vertices[vertexIdx++], vertices[vertexIdx++], vertices[vertexIdx++]);
            }
        }
    glEnd();
}
